...
export async function GET(request:Request) :Promise<NextResponse>{
    ...
    const responsetv = await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${TMDB_API_KEY}&language=en-US&page=1&${filter}`);
    ...
    const URL_DB= `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&language=en-US&page=1&${filter}`;
    const responseM = await fetch(URL_DB);
    console.log(URL_DB);
    if (!responseM.ok) {
        const errorData = await responsetv.json();
        console.error('Erreur TMDB:', errorData);
